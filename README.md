...
本仓库本身不含模型代码，是 **ACE-Step 的包装层 + 12 Tab 统一控制台**(历史索引、自动后处理、LoRA 数据/管理、批量生成、A/B 对比、Prompt 工作室、Song→DAW 导出、封面&字幕、续写&翻唱、一键 MIDI、发行打包)。

## 快速开始

### Windows(推荐)

最简单玩法是只双击 **`start-manager.bat`**,然后在总控台里点按钮启停其它服务。

或者直接用对应 .bat:

| 双击文件 | 端口 | 作用 |
|---|---|---|
| `start-manager.bat` | 7862 | 🆕 **总控台**(启停 + 监控其它服务,看 GPU/日志) |
| `start-ui.bat` | 7860 | ACE-Step 主 UI(生成新歌, 调 ACE-Step 自带的官方启动器) |
| `start-lab.bat` | 7861 | **统一控制台**(12 Tab,见下方表格) |

三个互不打扰,可同时开。**新用户直接用总控台**,不用记其它 .bat。
...
# 统一控制台 UI(http://localhost:7861, 12 Tab)
...
| 🎼 一键 MIDI | 整曲直接出 MIDI + BPM + 段落 marker,跳过 stem 分离 | 「⬅ 用历史 Tab 选中的歌」/ 拖文件 |
| 📦 发行打包 | 打包 zip (wav+mp3+cover+lrc+mid+meta) + chorus 自动剪短版 | 「⬅ 用历史 Tab 选中的歌」 |
